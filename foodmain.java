...
public class FoodApp {
    ...
    public static void main(String[] args) {
        try (Scanner scanner = new Scanner(System.in)) {
            Map<String, Food> foodMap = initFoods();

            // Input weight
            System.out.print("Enter weight in pounds (e.g., 2.0) to calculate calories and protein: ");
            while (!scanner.hasNextDouble()) {
                System.out.print("Invalid input. Please enter a valid weight in pounds: ");
                scanner.next(); // Consume the invalid input
            }
            double weight = scanner.nextDouble();
            scanner.nextLine(); // Consume newline

            System.out.printf("Calories and protein for %.2f pounds:%n", weight);
            System.out.println("Enter food name or type 'exit' to quit:");

            while (true) {
                String foodName = scanner.nextLine().toLowerCase().trim();

                if (foodName.equals("exit")) {
                    break;
                }

                query(foodMap, foodName, weight);

                System.out.print("compare? (yes/no): ");
                String compareChoice = scanner.nextLine().toLowerCase();

                if (compareChoice.equals("yes")) {
                    String secondFoodName = scanner.nextLine().toLowerCase().trim();
                    compare(foodMap, foodName, secondFoodName, weight);
                }
            }
        }
    }
